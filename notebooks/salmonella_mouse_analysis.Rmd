...
nonlinear_trans <- function(left, right, zoom){
...
results_analysed_3 <- results %>%
  ...
  mutate(
    ...
    normflux = coalesce(norm1, norm2, norm3, norm4)
  ) %>%
  select(-norm1, -norm2, -norm3, -norm4) %>%
  ungroup %>%
...
horizontal_point_summary <- results_analysed_3 %>%
  ...
  scale_x_continuous(labels=scales::percent, trans=nonlinear_trans(1.1,1.3,20)) +
    geom_point(aes(colour=group, shape=group), alpha=0.5) +
    labs(x='Normalized Flux', y='Reaction Abbreviation') +
    theme_bw() + 
    geom_tile(aes(width=0.1, x=3.8, y=abbreviation, fill=letters[as.numeric(as.factor(Subsystem)) %% 8+1]), show.legend = FALSE) +
    geom_text(aes(x=3.8, label = pathway, group=Subsystem), check_overlap=TRUE, data=(. %>% group_by(Subsystem) %>% mutate(pathway = ifelse(abbreviation==nth(abbreviation,round(n()/2)),Subsystem,NA))), hjust=1, alpha=0.5, size=2.5) +
