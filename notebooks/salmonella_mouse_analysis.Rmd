...
models_with_targets <- models_with_expression %>%
  group_by(group) %>%
  nest %>%
  mutate(data = map(data, find_flux_variability_df, folds=folds, do_minimization=TRUE)) %>%
  unnest() %>%
  ...
  mutate(
    adjustment = ifelse(is.na(activation), abs(baseflux)*0.1+0.1, abs(activation-1)*abs(baseflux)*0.1+0.1),
...
results_analysed_3 <- results %>%
  ...
  mutate(
    ...
    boring_1 = all(near(flux, inputflux)),
    boring_2 = all(near(flux, baseflux)),
...
horizontal_point_summary <- results_analysed_3 %>%
    filter(stable_1, stable_2, !boring_1, !boring_2) %>% top_n(30*4, interest) %>%
...
rate_tables <- results_analysed_3 %>%
  filter(stable_1, stable_2, !boring_1, !boring_2) %>% top_n(20*4, interest) %>%
  group_by(group) %>%
  nest() %>%
  mutate(data = map(data, function(x){
